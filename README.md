...
> 1. 安装程序不可用，将无限循环。请自行在其他主机安装后将config下文件拷贝到仓库，然后部署。
> 2. 新建/移动等文件操作，依赖异步更新，前端将无法接收操作成功/失败提醒。需要自行刷新查看操作结果。
...
- 全局搜索/当前页过滤
- 删除/新建/上传/重命名/剪切等，基本的onedrive文件操作。仅管理员可用
- 分享链接
- aria2离线上传
